...
interface DetailsPanelProps {
    ...
    onResetTracking?: (taskId: string) => void;
}

export const DetailsPanel = ({
    editingTask, setEditingTask, isDark, handleToggleTask, handleUpdateTaskDetail,
    handleDeleteTask, currentTaskSubtasks, handleAddSubtask, setChecklistModalTask, panelRef, onReparentTask, onToggleTracking, onResetTracking
}: DetailsPanelProps) => {
    ...
    const [liveSeconds, setLiveSeconds] = React.useState(editingTask?.timeSpent || 0);

    React.useEffect(() => {
        let interval: NodeJS.Timeout;
        if (editingTask?.trackingStartedAt) {
            interval = setInterval(() => {
                const elapsed = Math.floor((Date.now() - editingTask.trackingStartedAt!) / 1000);
                setLiveSeconds((editingTask.timeSpent || 0) + elapsed);
            }, 1000);
        } else {
            setLiveSeconds(editingTask?.timeSpent || 0);
        }
        return () => {
            if (interval) clearInterval(interval);
        };
    }, [editingTask?.trackingStartedAt, editingTask?.timeSpent]);
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                        ...
                        <div className="mb-6">
                            <div className="flex items-center justify-between mb-2">
                                <label className={`text-[10px] font-bold uppercase block ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>Prioridad & Seguimiento</label>

                                {onToggleTracking && (
                                    <button
                                        onClick={() => onToggleTracking(editingTask)}
                                        className={`px-2 py-1 flex items-center gap-1.5 rounded-full text-xs font-bold transition-all ${editingTask.trackingStartedAt ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20 shadow-[0_0_12px_rgba(239,68,68,0.2)]' : isDark ? 'bg-zinc-800 text-emerald-500 hover:bg-zinc-700' : 'bg-gray-100 text-emerald-600 hover:bg-gray-200'}`}
                                    >
                                        {editingTask.trackingStartedAt ? (
                                            <>
                            ...
                            {/* Tracker active state info */}
                            {(editingTask.timeSpent! > 0 || editingTask.trackingStartedAt) && (
                                <div className={`mb-3 p-3 rounded-lg border flex items-center justify-between ${editingTask.trackingStartedAt ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500' : isDark ? 'bg-zinc-900/50 border-zinc-800/50 text-zinc-400' : 'bg-gray-50 border-gray-100 text-gray-500'}`}>
                                    <div className="flex items-center gap-2">
                                        <Timer size={16} />
                                        <span className="text-xs font-medium uppercase tracking-wider">Tiempo Dedicado</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <div className="font-mono text-xl font-black tracking-tight">{formatTime(liveSeconds)}</div>
                                        {onResetTracking && (
                                            <button
                                                onClick={() => {
                                                    if (window.confirm('¿Seguro de reiniciar el tiempo de esta tarea?')) {
                                                        onResetTracking(editingTask.id);
                                                    }
                                                }}
                                                className={`p-1.5 rounded-full transition-colors ${isDark ? 'hover:bg-zinc-800 text-zinc-500 hover:text-red-400' : 'hover:bg-gray-200 text-gray-400 hover:text-red-500'}`}
